...
//#include <libtorrent/session_impl.hpp>
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            while (!h.is_seed()) {
                ...
                if (self.progressPrint) {
                    ...
                    NSLog(@" %@ %.2f%% (下载速度: %.1fkb/s  上传速度: %.1fkB/s  连接: %d)",statusStr[status.state],status.progress*100,status.download_rate/1000.0,status.upload_rate/1000.0,status.num_peers);
                    for (BTFile *file in self.torrentInfo.files) {
...
+ (void)downloadTorrentFromMagnet:(NSString *)magnet complete:(void (^)(NSString *))complete {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        while (!h.has_metadata()) {
            libtorrent::torrent_status status = h.status();
            NSLog(@"种子下载中(速度: %.1fkb/s 连接: %d)",status.download_rate/1000.0,status.num_peers);
            [NSThread sleepForTimeInterval:1];
        }
        s.pause();
